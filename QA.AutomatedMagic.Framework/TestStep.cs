        ...
        public override void Execute()
        {
            ...
            catch (Exception ex)
            {
                if (IsSkippedOnFail)
                {
                    ...
                    Log.WARN($"Error occurred during step initialization", ex);
                    Log.WARN($"Execution of {this} completed with status: {ItemStatus}");
                    Parent.Log.WARN($"Execution of {this} completed with status: {ItemStatus}. Error occurred during step initialization", ex);
                    return;
                }
                else throw ex;
            }
